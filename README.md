...
There are lots of functions have their own commands, e.g., $\lg (x)$, $\log (x)$, $\sin (x)$, $\cos (x)$ and so on. Please don't write `lg(x)`, and instead write `\lg(x)`. Similarly, same for `\log(x)`, `\sin(x)`, `\cos(x)` and so on.
